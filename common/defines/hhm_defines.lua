...
NDefines.NIndustrialOrganisation.FUNDS_FOR_SIZE_UP = 800								-- Funds needed for a MIO to increment its size and get points to unlock traits
NDefines.NIndustrialOrganisation.FUNDS_FOR_SIZE_UP_LEVEL_FACTOR = 200						-- How much each level mutliplies the funds for size up 
...
NDefines.NIndustrialOrganisation.UNLOCKED_TRAITS_PER_SIZE_UP = 0							-- Number of points for unlocking traits obtained when the MIO increments its size
...
NDefines.NIndustrialOrganisation.FUNDS_FOR_RESEARCH_COMPLETION_PER_RESEARCH_COST = 0     	-- Funds added to MIO when the Design Team has completed a research, multiplied by research_cost in technology template
...
NDefines.NIndustrialOrganisation.FUNDS_FROM_MANUFACTURER_PER_IC_PER_DAY = 0.0				-- Funds added to MIO when a manufacturer is attached to a production line. Added every day proportional to IC produced.
