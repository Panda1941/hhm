...
NDefines.NAir.NAVAL_STRIKE_CARRIER_MULTIPLIER = 10              -- damage bonus when planes are in naval combat where their carrier is present (and can thus sortie faster and more effectively)
...
NDefines.NIndustrialOrganisation.FUNDS_FOR_SIZE_UP_LEVEL_POW = 2.15							-- the power we applie to the mio size when calculating funds to level up. 	
...
NDefines.NIndustrialOrganisation.FUNDS_FROM_MANUFACTURER_PER_IC_PER_DAY = 0.08				-- Funds added to MIO when a manufacturer is attached to a production line. Added every day proportional to IC produced.
